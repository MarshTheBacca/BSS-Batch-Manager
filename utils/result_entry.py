...
from .introduce_defects.utils.other_utils import (
    calculate_angles_around_node,
    pbc_vector,
)
from .other_utils import (
    get_polygon_area,
    get_polygon_area_estimate,
    string_to_value,
)
...
short_name_to_var: dict[str:Var] = {
    "Mini_ring_size": IntVar(name="Minimum ring size", lower=3),
...
def get_changing_vars(job_path: Path) -> list[Var]:
    ...
    try:
        for short_name, string_value in changing_vars_dict.items():
            var: Var = copy.deepcopy(short_name_to_var.get(short_name))
            if var is None:
...
def get_bond_lengths(node_coords: np.ndarray, adjacencies: list[list[int]], dimensions: np.ndarray) -> np.ndarray:
    ...
    return np.array([np.linalg.norm(pbc_vector(node_coords[node], node_coords[neighbour], dimensions)) for node, neighbours in enumerate(adjacencies) for neighbour in neighbours])
...
def get_summary_info(job_path: Path) -> tuple[int, int, int, int, int, float, float, float, bool]:
    ...
    return (num_attempted, num_accepted, num_failed_angle_checks, num_failed_bond_length_checks, num_failed_energy_checks, acceptance_rate, total_run_time, average_time_per_step, consistent)
...
class ResultEntry:
    ...
    def from_path(job_path: Path) -> ResultEntry:
        ...
        (bond_length_variance, angle_variance, ring_area_variance, ring_size_variance, pearsons, p6) = get_disorder_info(job_path, dimensions, {ring_id})
        (num_attempted_switches, num_accepted_switches, num_failed_angle_checks, num_failed_bond_length_checks, num_failed_energy_checks, acceptance_rate, total_run_time, average_time_per_step, consistent) = get_summary_info(job_path)

        return ResultEntry(
            job_path,
            changing_vars,  # path, changing vars
            pore_size,
            pore_distance,  # pore size, pore distance
            float(last_data_line[2]),
            float(last_data_line[3]),  # energy, entropy
            num_nodes,
            num_rings,  # num nodes, num rings
            dimensions,  # dimensions
            ring_area,
            ring_area_estimate,  # ring area, ring area estimate
            bond_length_variance,
            angle_variance,  # bond length variance, angle variance
            ring_area_variance,
            ring_size_variance,  # ring area variance, ring size variance
            pearsons,
            p6,
            float(last_data_line[5]),  # pearsons, aboave weaire
            num_attempted_switches,
            num_accepted_switches,  # num attempted switches, num accepted switches
            num_failed_angle_checks,
            num_failed_bond_length_checks,  # num failed angle checks, num failed bond length checks
            num_failed_energy_checks,
            acceptance_rate,  # num failed energy checks, acceptance rate
            total_run_time,
            average_time_per_step,  # total run time, average time per step
            consistent,
        )  # consistent
    ...
    def from_string(string: str) -> ResultEntry:
        ...
        return ResultEntry(
            ...
            changing_vars,  # path, changing vars
            int(data[1]),
            float(data[2]),  # pore size, pore distance
            float(data[3]),
            float(data[4]),  # energy, entropy
            int(data[5]),
            int(data[6]),  # num nodes, num rings
            dimensions,  # dimensions
            float(data[8]),
            float(data[9]),  # ring area, ring area estimate
            float(data[10]),
            float(data[11]),  # bond length variance, angle variance
            float(data[12]),
            float(data[13]),  # ring area variance, ring size variance
            float(data[14]),
            float(data[15]),
            float(data[16]),  # pearsons, p6, aboave weaire
            int(data[17]),
            int(data[18]),  # num attempted switches, num accepted switches
            int(data[19]),
            int(data[20]),  # num failed angle checks, num failed bond length checks
            int(data[21]),
            float(data[22]),  # num failed energy checks, acceptance rate
            float(data[23]),
            float(data[24]),  # total run time, average time per step
            data[25].lower() == "true",
        )  # consistent
    ...
    def plot_ring_sizes(self, smoothing: int = 20) -> None:
        ...
        all_ring_sizes = sorted(set(ring_size for distribution in bss_output_data.ring_sizes for ring_size in distribution.keys()))
    ...
    def plot_bond_length_distribution(self, num_bins: int | None = None) -> None:
    ...
    def plot_angle_distribution(self, num_bins: int | None = None) -> float:
    ...
    def plot_all_per_step(self, smoothing: int = 20) -> None:
        ...
        _, variances = self.get_ring_size_variance_data_fast(smoothing)
        ax = plt.gca()
        ax.plot(steps, relative_energies, label=r"Relative Energy ($\mathdefault{E_h Node^{-1}}$)")
